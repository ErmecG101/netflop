...
$select_generico = 'select c.concod, c.contit, c.conimg,c.consip,c.connotimdb, p.pernom, p.percod, GROUP_CONCAT(g.gennom) from conteudos as c inner join generos as g inner join perfil as p inner join gengost as gg inner join gencon as gc
where c.concod = gc.codcon and gc.codgen = gg.codgen and g.gencod = gg.codgen and gg.codper = p.percod  and p.percod = '.$_SESSION['percod'].' group by c.contit';
        ...
        <div class="container">
            <div class="row">
                ...
                for ($i = 1; $i <= $maxnum; $i++) : ?>
                    ...
                    <?php $ln = mysqli_fetch_assoc($result_generico);
                    $y++;
                        if ($ln) :
                    ?>
                            <div class="card-dark" style="width: 20rem;">
                                <form method="POST" action="./processos/resolvedorcon.php">
                                    <p style="background-color: rgb(48, 57, 68);">Por você gostar de: <?php echo str_replace(',',', ', $ln['GROUP_CONCAT(g.gennom)']) ?></p>
